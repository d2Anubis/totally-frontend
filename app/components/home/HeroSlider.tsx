...
const HeroSlider = () => {
  ...
  if (loading) {
    return (
      <div className="relative w-full overflow-hidden rounded-xl mb-3 md:mb-6 px-4 md:px-0">
        <div
      ...
      <div className="relative w-full overflow-hidden rounded-xl mb-3 md:mb-6 px-4 md:px-0">
        <div className="relative w-full" style={{ height: `${imageHeight}px` }}>
          {slides.map((slide, index) => (
            <div
              key={slide.id}
              className={`absolute top-0 left-0 w-full h-full rounded-2xl transition-opacity duration-500 ease-in-out ${
                index === currentSlide ? "opacity-100 z-10" : "opacity-0 z-0"
              }`}
            >
              <div 
                className={`w-full h-full rounded-2xl ${slide.buttonUrl ? 'cursor-pointer' : ''}`}
                onClick={() => {
                  if (slide.buttonUrl) {
